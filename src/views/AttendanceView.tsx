...
export const AttendanceView = ({ setCurrentView }: { setCurrentView: (v: string) => void }) => {
  ...
  const handleAction = (type: 'check-in' | 'check-out') => {
    ...
    if (type === 'check-in') {
      ...
      setMessage({ type: 'success', text: `${selectedUser.name} checked in at ${newRecord.checkIn}` });
    } else {
      setAttendanceRecords(prev => prev.map(r => 
        r.userId === selectedUser.id && r.status === 'active' 
          ? { ...r, checkOut: currentTime.toLocaleTimeString(), status: 'completed' } 
          : r
      ));
      setMessage({ type: 'success', text: `${selectedUser.name} checked out at ${currentTime.toLocaleTimeString()}` });
    }
  ...
  const isUserCheckedIn = (userId: string) => {
    return attendanceRecords.some(r => r.userId === userId && r.status === 'active');
  };
    ...
    <div className="flex-1 overflow-y-auto p-8 bg-grid-pattern flex flex-col items-center justify-center min-h-full relative overflow-hidden">
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="flex flex-col">
          ...
          >
            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-secondary mb-3">OPERATIONAL CHRONOGRAPH</p>
          ...
          >
            <div className="bg-surface-container/40 backdrop-blur-2xl rounded-[2rem] p-6 border border-outline-variant/10 shadow-2xl">
              ...
              <div className="space-y-5">
                {attendanceRecords.slice(-3).reverse().map((record, i) => {
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[8px] font-bold text-on-surface-variant uppercase tracking-widest mt-0.5">
                          {record.status === 'active' ? (
                            <span className="text-tertiary">IN • {record.checkIn}</span>
                          ) : (
                            <span className="text-secondary">OUT • {record.checkOut}</span>
                          )}
                        </p>
                      </div>
                    </motion.div>
