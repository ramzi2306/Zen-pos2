...
async def _to_public(user: UserDocument) -> UserPublic:
    ...
    return UserPublic(
        ...
        is_system=role.is_system if role and getattr(role, "is_system", None) is not None else False,
        in_order_prep=role.in_order_prep if role and getattr(role, "in_order_prep", None) is not None else True,
        image=user.image or "",
