...
export function buildReceiptHtml(d: ReceiptData): string {
  ...
  const itemRows = d.items.map(item => {
    ...
    return `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;font-weight:700;font-size:15px;"><span>${item.quantity}x ${item.name}</span><span style="white-space:nowrap;margin-left:8px;">${formatCurrency(item.lineTotal)}</span></div>${noteStr ? `<div style="padding-left:16px;font-size:15px;font-weight:900;color:#000;text-transform:uppercase;">${noteStr}</div>` : ''}</div>`;
  }).join('');

  const hasCustomer = d.customer && (d.customer.name || d.customer.phone || d.customer.address);
  const customerSection = hasCustomer
    ? `${SEP}<div style="font-weight:900;font-size:16px;margin-bottom:4px;border-bottom:1px solid #000;padding-bottom:2px;">CUSTOMER DETAILS</div><div style="font-size:15px;font-weight:bold;">${d.customer!.name ? `<div>${d.customer!.name}</div>` : ''}${d.customer!.phone ? `<div>${d.customer!.phone}</div>` : ''}${d.customer!.address ? `<div>${d.customer!.address}</div>` : ''}</div>`
    : '';
  ...
  // If paymentStatus is exactly 'Paid' or covered by logic:
  const paymentStatusDisplay = (d as any).paymentStatus?.toUpperCase() || (paidAmount > 0 || (d as any).paymentStatus === 'Paid' ? 'PAID' : 'UNPAID');
  
  const paymentStatusSection = `<div style="text-align:center;font-size:18px;font-weight:900;margin:6px 0;padding:4px;border:2px dashed #000;">STATUS: ${paymentStatusDisplay}</div>`;

  const changeAmt = paidAmount > d.total ? paidAmount - d.total : 0;
  const paidSection = paidAmount > 0
    ? `${SEP}<div style="display:flex;justify-content:space-between;font-size:14px;font-weight:bold;"><span>Cash Paid:</span><span>${formatCurrency(paidAmount)}</span></div><div style="display:flex;justify-content:space-between;font-weight:900;font-size:15px;color:#000;"><span>Change:</span><span>${formatCurrency(changeAmt)}</span></div>`
    : '';

  const showQr = branding.printQrCode && !!d.trackingUrl;
  const qrSvg = showQr ? generateQrSvg(d.trackingUrl!, 110) : '';
  const qrSection = showQr && qrSvg
  ...
  const notesSection = d.notes
    ? `${SEP}<div style="font-size:15px;font-weight:900;color:#000;border:1px solid #000;padding:4px;margin-top:4px;">NOTE: ${d.notes}</div>`
    : '';
